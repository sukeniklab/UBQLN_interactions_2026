...
def load_mod(mod, trials):
    ...
    for trial in trials:
        rg = 10 * np.load(
            f"../../Data/old_pipeline/CALVADOS3COM_2.0_MD_gpu_trial{trial}_Dsk2_{mod}/0/Rg_traj.npy")
        arrays.append(rg)
...
variants = {
    "dTH1": {"mods": ["deltaTH1", "deltaTH1_bound"], "exp_rg": 35.7,  "err": 0.1, "color": "green"},
    "dTH2": {"mods": ["deltaTH2", "deltaTH2_bound"], "exp_rg": 37.47, "err": 0.1, "color": "fuchsia"},
    "dTH3": {"mods": ["deltaTH3", "deltaTH3_bound"], "exp_rg": 40.2,  "err": 0.1, "color": "orange"},
}
...
for ax, (var_name, config) in zip(axes, variants.items()):
    ...
    ax.text(50, 0.05, f"{var_name}={ens_rg:.1f}±{ens_rg_std:.1f} Å",
...
plt.savefig('Dsk2_deletions_rg_stacked.svg', dpi=300, bbox_inches='tight')
plt.close()